...
export class AuthService {
    ...
    async login(data : LoginDto){
        const employer = await this.prisma.employer.findUnique({ where: { email: data.email }});
        if (!employer) {
            throw new UnauthorizedException('Invalid Credentials');
        }  
        const isPasswordValid = await bcrypt.compare(data.password, employer.passwordHash);
        if (!isPasswordValid){
            throw new UnauthorizedException('Invalid Credentials');
        }
